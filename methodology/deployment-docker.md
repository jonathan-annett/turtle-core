...
- **Claude Code authentication.** `claude-code` (installed via apt as in §5.1) uses OAuth interactively on first run, storing credentials in `~/.claude/`. The architect persists these via the `architect-claude-state` named volume — log in once, resumed forever. Ephemeral containers (planner, coder subshells, auditor) cannot do interactive OAuth — fresh containers have no `~/.claude/`, no browser, and would block on the auth prompt. Two workable options for ephemerals: (a) set `ANTHROPIC_API_KEY` in the container env, which `claude-code` accepts in lieu of OAuth; or (b) mount a credentials directory (e.g. `~/.claude` from the host or a shared named volume) read-only into the ephemeral container so it inherits the architect's session. Option (a) is simpler and recommended; it's what the per-pair env file should carry alongside `COMMISSION_TOKEN`. Document this in your `.env.example` and the README.

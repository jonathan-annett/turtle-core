...
**Version 2.5.** Code migration agent added as a sub-agent of the onboarder. The onboarder dispatches it once per brownfield onboarding to perform structural review of the source materials at `/source` (lint, syntax, dependency resolution, import-graph closure, orphan-file / stale-directory detection — not build, not behavioural test). The agent produces a survey report at `briefs/onboarding/code-migration.report.md`, severity-graded for-architect's-attention rather than gate-shaped. The agent's image is composed via the v2.4 platform-composition mechanism, with platforms declared in the migration brief that the onboarder authors during its phase-1 elicitation (rather than in a section brief — there is no project superset yet at onboarding time). See §3.3, §4 "Code migration agent", §7.9, §7.10, §8 step 0, §9.
...
| `code-migration` | main repo (all paths); brownfield source materials at `/source` (read-only mount, same as the onboarder) | main repo: `refs/heads/main` only, restricted to the single path `briefs/onboarding/code-migration.report.md` (enforced by hook). Single-shot per onboarding: the operator-side dispatch helper enforces single-dispatch; the path restriction is defence in depth. No access to the auditor repo. Sub-agent of the onboarder — not commissioned independently. |
...
- Collaborates with **sub-agents** — the *code migration agent* (structural code review against target-language toolchains; **active** since v2.5) and the *history migration agent* (reconstruction from preserved transcripts and artifacts, for types 3 and 4; not yet shipped). These sub-agents are descriptive-name sub-roles whose scope lives inside the onboarder's run, distinct in name from the profession-name top-level roles. The code migration agent is dispatched host-side by the operator-side entry script between the onboarder's phase-1 (elicitation + migration brief) and phase-3 (findings integration + final handover); the onboarder itself does not have docker access. Until the history migration agent ships, section 4 of the handover is filled by the onboarder with operator-acknowledged "no automated review run" notes plus pointer-level observations from a single read-through, per the handover template.
...
- Runs across two operator-facing phases driven by the operator-side entry script (phase 1: elicitation + migration brief + draft handover; phase 3: findings integration + final handover). Between them, phase 2 dispatches the **code migration agent** host-side (see below).
- Discharges. Its container is removed. Its output (the handover brief) is consumed by the architect on first attach; see §8.

### Code migration agent (ephemeral, sub-agent of onboarder, one per onboarding)
- Exists only inside an onboarding run. **Not a top-level methodology role**; not commissioned independently. Distinct in name from the profession-name top-level roles per the methodology's "X migration agent" sub-agent naming convention.
- Started host-side by the operator-side dispatch helper (the deployment variant supplies its name; for variant A it is `infra/scripts/dispatch-code-migration.sh`, invoked transparently from `./onboard-project.sh` between onboarder phases 1 and 3).
- Receives the brownfield source materials via a read-only mount at `/source` (same mount the onboarder reads from). Its image is composed at dispatch time with the platforms declared in the migration brief's "Required platforms" field (the v2.4 platform-composition mechanism), and its `--allowed-tools` is parsed from the brief's "Required tool surface" field at startup (the v2.2 / s011 mechanism).
- Performs **structural review only**: lint, syntax, dependency resolution (`pip install --dry-run`, `npm install --dry-run`, `cargo check`, `go vet`), import-graph closure, orphan-file / stale-directory detection. Does not build the project. Does not execute the project's behavioural tests. Does not dispatch further sub-agents — the depth-stop sits here; tooling the survey needs goes in the agent's own tool surface, not in a child.
- Runs non-interactive (`claude -p`, no operator-in-loop). Survey produces no operator decisions; questions the agent cannot resolve become open questions in the migration report for the architect.
- Produces exactly one artifact: the migration report at `briefs/onboarding/code-migration.report.md` in the main repo, conforming to `methodology/code-migration-report-template.md`. Six sections: brief echo, per-component intent, structural completeness, findings (severity-graded HIGH / LOW / INFO, framed for the architect's attention — **not gate-shaped**), operational notes, open questions.
- Single-shot per onboarding. Re-dispatch is out of scope; if a re-survey is genuinely needed, the operator tears the substrate down and re-onboards.
- May write only to `briefs/onboarding/code-migration.report.md` on `main`. The git-server's update hook enforces the path restriction; the dispatch helper enforces the single-shot envelope. No access to the auditor repo at all.
- Discharges. Its container is removed. Its output (the migration report) is consumed by the onboarder in phase 3, which folds findings into section 3 of the handover brief.
...
### 7.9 Migration brief (onboarder → code migration agent)
Path: `briefs/onboarding/code-migration.brief.md`. Authored by the onboarder during phase-1 elicitation; consumed by the code migration agent at dispatch. Project-scoped (under `briefs/onboarding/`), not section-scoped. The full specification of its contents lives in `methodology/code-migration-brief-template.md` ("must contain" form). Headlines:
- **Objective.** Structural review of `/source`; produce the migration report. Restate-in-your-own-words contract for the agent's brief echo.
- **Available context.** Pointers to `/source`, the role guide, the report template, the project-type hint, any preliminary onboarder observations worth carrying across.
- **Required platforms.** Same fenced grammar as §7.2's section-brief field. The onboarder either inferred this set from `/source` signals or received it from the operator via the entry-script's override flag. Unlike §7.2's subset-of-superset semantics, this field is **authoritative on its own** — TOP-LEVEL-PLAN.md does not exist at onboarding time (the architect drafts it on first attach).
- **Required tool surface.** Same fenced grammar as §7.3 / §7.6. Survey-grade probes appropriate to the declared platforms (`pip install --dry-run`, `ruff`, `mypy` for python; `cargo check`, `clippy` for rust; etc.) plus `Read`, `Edit`, `Write` scoped to the report, plus `git add/commit/push` patterns.
- **Output destination.** Pointer to `briefs/onboarding/code-migration.report.md` plus the canonical commit-message convention.
- **Reporting requirements.** Pointer to the report template (§7.10) plus the section-headings contract.

### 7.10 Migration report (code migration agent → onboarder)
Path: `briefs/onboarding/code-migration.report.md`. Authored by the code migration agent during phase-2 dispatch; consumed by the onboarder in phase-3 findings integration. Project-scoped, preserved on disk indefinitely as a historical artifact. The full specification lives in `methodology/code-migration-report-template.md` ("must contain" form). Shape differs from §7.5 section-report: this report is **survey feedstock**, not work-completed. Six sections:
- **Brief echo.** Agent's restatement of the commissioning task.
- **Per-component intent.** Top-level subtrees the agent inspected — purpose, language(s), apparent state — confidence-calibrated.
- **Structural completeness.** Outcome of import-graph / dependency-resolution / orphan / dangling-reference / stale-directory probes.
- **Findings.** Severity-graded (HIGH / LOW / INFO, same legend as `FINDINGS.md`) but **not gate-shaped** — framed as "for architect's attention" with a suggested next-step rather than a "must-fix" demand.
- **Operational notes.** Things that would surprise an architect bootstrapping the project (unusual layouts, vendored upstreams, generated code committed deliberately, etc.).
- **Open questions.** Items the agent could not resolve from structural review alone, addressed to the architect (and, when it ships, the history migration agent).

The onboarder folds the report's findings into section 3 of the handover brief during phase 3, referencing the report by path for deeper reads.

---

## 8. Lifecycle

0. **(Brownfield projects only) Onboard.** Before the architect attaches for the first time, the human runs the operator-side onboarding script. The substrate drives a three-phase flow:

   1. **Phase 1 — elicit and commission.** The onboarder imports the brownfield source tree as the initial commit on `main`. Platforms are inferred from canonical signal files at `/source` (or supplied directly via `--platforms=<csv>` on the entry script). The onboarder synthesises and elicits with the operator, confirms the platform set, and produces two artifacts on `main`: the migration brief at `briefs/onboarding/code-migration.brief.md` (commissioning artifact for the code migration agent; see §7.9) and a draft handover at `briefs/onboarding/handover.draft.md` with section 3 left as a TODO placeholder. The onboarder discharges.

   2. **Phase 2 — dispatch the code migration agent.** The host's dispatch helper (`infra/scripts/dispatch-code-migration.sh` for variant A) composes the agent's image with the platforms declared in the migration brief (the v2.4 mechanism; see §9), validates the brief's tool surface against the composed image, runs the agent autonomously (no operator-in-loop; see §4 "Code migration agent"), and waits for the agent to commit the migration report at `briefs/onboarding/code-migration.report.md` (see §7.10).

   3. **Phase 3 — integrate and finalise.** A fresh onboarder container reads the draft handover and the migration report, integrates the report's findings into section 3 of the handover (and cross-integrates operational notes / open questions into other sections as warranted), and produces the final `briefs/onboarding/handover.md`. The onboarder discharges.

   On the architect's first attach, the substrate's architect entrypoint detects the handover (and the absence of `SHARED-STATE.md` — first-attach signal) and seeds the architect's first claude session against it, so step 1 below begins from the synthesis the onboarder and the code migration agent produced rather than from a blank slate. Greenfield projects skip step 0 entirely.
...
**Platform composition (v2.4, extended in v2.5).** Before each dispatched-agent commissioning event, the substrate resolves the agent image's target platforms, composes a hash-tagged image carrying those platforms, and validates that every Bash-anchored binary in the brief's tool surface is present on the composed image's PATH. The mechanism is universal across all dispatched roles; the differences are in how platforms are resolved per role:

- **Planner, coder-daemon, auditor.** Platforms resolve from the section brief's or audit brief's optional "Required platforms" field, then the TOP-LEVEL-PLAN.md `## Platforms` project superset, then (as a v2.3-compat fallback) the substrate-setup platform record at `.substrate-state/platforms.txt`. Subset enforcement applies: section ⊆ project superset.

- **Onboarder.** Commissions with an empty platform set by design — its work is brief synthesis, not building. The composition produces the static template build via the default-platform no-op semantics.

- **Code migration agent (v2.5).** Platforms come from the **migration brief's** "Required platforms" field, authored by the onboarder during phase-1 elicitation (with the operator either confirming the inferred set or supplying it via the entry script's override flag). The migration brief is project-scoped, not section-scoped — there is no TOP-LEVEL-PLAN.md project superset to subset against at onboarding time (the architect drafts it on first attach, after the onboarder discharges). The migration brief's field is therefore **authoritative on its own**: the dispatch helper parses it directly and composes against the resulting set, with no subset enforcement.

The substrate reuses each composed image across commissions when the declaration is unchanged, and rebuilds only when the declaration or the underlying platform registry changes (the cache key includes the sha256 of each selected platform YAML's bytes). The composition does not promise byte-level image reproducibility (upstream apt/pip/npm versions vary between rebuilds); the platform registry entry is the deterministic input, not the rendered image's content bytes. A missing binary in the tool-surface validation pass fails commission cleanly with an error naming the missing binaries and the platform set used.

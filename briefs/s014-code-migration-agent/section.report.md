...
The override-during-implementation latitude was used once and recorded explicitly below (B.6 dispatch hosting). Everything else hewed to the brief's seven recommendations as written.
...
## Override-during-implementation deviations from the brief

1. **B.6 dispatch hosting (host-side, not in-onboarder).** The brief reads "the dispatch call" as sitting inside the onboarder's entrypoint. Host-side dispatch via `./onboard-project.sh` avoids mounting `/var/run/docker.sock` into the onboarder container (real privilege elevation). Trade-off: split-phase interactive UX. Detail in B.6 above.
